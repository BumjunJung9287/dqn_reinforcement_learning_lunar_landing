...
class DeepQNetwork(nn.Module):
    def __init__(self, lr, input_dims, fc1_dims, fc2_dims, n_actions):
        ...
        self.device = T.device('cuda:0' if T.cuda.is_available() else 'cpu:0s')
...
class Agent(object): # subject which do the actions
    ...
    def learn(self):
        if self.mem_cntr > self.batch_size:
            ...
            #print("error", batch_index, action_indices, reward_batch, terminal_batch,
            #self.gamma,T.max(q_next, dim=1)[0], q_target.shape)
            #print(batch_index, action_indices)
            #print(action_batch, action_values, batch_index, action_indices)
